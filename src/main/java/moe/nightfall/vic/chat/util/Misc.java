...
public class Misc
{
    ...
    public static String[] parseModt(String modt, EntityPlayerMP player)
    {
        ...
        if (modt.contains("%TIME%"))
            modt = modt.replaceAll("%TIME%", new SimpleDateFormat("HH:mm").format(Calendar.getInstance().getTime()));
        if (modt.contains("%ONLINE%"))
            modt = modt.replaceAll("%ONLINE%", String.valueOf(player.mcServer.getCurrentPlayerCount()));
        if (modt.contains("%ONLINE_MAX%"))
            modt = modt.replaceAll("%ONLINE_MAX%", String.valueOf(player.mcServer.getMaxPlayers()));
        if (modt.contains("%DIM%"))
            modt = modt.replaceAll("%DIM%", String.valueOf(player.worldObj.provider.dimensionId));
        if (modt.contains("%DIM_NAME%"))
            modt = modt.replaceAll("%DIM_NAME%", player.worldObj.provider.getDimensionName());
    ...
    {
        for (EntityPlayerMP entity : (List<EntityPlayerMP>) MinecraftServer.getServer().getConfigurationManager().playerEntityList)
            if (entity.getCommandSenderName().equalsIgnoreCase(player))
                return entity;
